...
def self_play_episodes(mdp: Connect4MDP, agent: DeepQAgent, episodes: int, eps: float) -> list:
    ...
    # GAME OVER. Gather experience tuples: (state, action, reward, next_state, done)
        ...
        states_actions_rewards = states_actions
        states_actions_rewards[::2] = map(lambda states_actions: states_actions + (p1_reward,), states_actions[::2])
        states_actions_rewards[1::2] = map(lambda states_actions: states_actions + (p2_reward,), states_actions[1::2])
